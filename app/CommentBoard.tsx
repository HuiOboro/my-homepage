...
export default function PhotoGalleryHomePage() {
  ...
  // 初始化从浏览器本地缓存（localStorage）加载留言，初始值为空（已删除小明）
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-extrabold text-slate-900 flex items-center gap-2">
              <span>💬</span> 互动留言板
              <span className="text-xs font-normal text-slate-400 bg-slate-100 px-2.5 py-1 rounded-full ml-2">
                {comments.length} 条留言
              </span>
            </h2>
          </div>
          ...
          <form onSubmit={handleAddComment} className="mb-8 space-y-3">
            <input 
              ...
              className="w-full px-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all select-text"
            />
            <textarea 
              rows={3}
              placeholder="给 Huioboro 留个言吧..." 
              value={contentInput}
              onChange={(e) => setContentInput(e.target.value)}
              className="w-full px-4 py-3 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all resize-none select-text"
              required
            />
            <div className="flex justify-end">
              <button 
                type="submit" 
                className="bg-teal-600 hover:bg-teal-700 text-white font-medium text-xs px-5 py-2.5 rounded-xl shadow-md shadow-teal-600/20 active:scale-95 transition-all"
              >
                发表留言 ✨
              </button>
            </div>
              ...
              comments.map((item) => (
                <div key={item.id} className="p-4 bg-slate-50/80 rounded-2xl border border-slate-100 flex items-start justify-between group">
                  ...
                  <button 
                    ...
                    className="text-[10px] text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity ml-2"
                    title="删除此留言"
                  >
                    删除
                  </button>
